...
- **Ascending/Descending Chimes** — Plays a custom 4-tone ascending success arpeggio when the target is found, and a sad 2-tone descending failure chime when a target is missing.
...
### ⚡ Global Interactions
- **Premium Landing Page** — Frosted glassmorphic navigation hub allowing users to select either the Searching or Sorting modules.
...
- **Dark/Light Mode** — Sleek glassmorphic dark interface by default, with an elegant light theme toggle.
- **Responsive Header** — Auto-aligns theme/sound controls to the top row and collapses text labels into compact icon buttons (`🏠`, `🔊`/`🔇`, `☀`/`🌙`) on mobile screens.
- **Dynamic Header Icons** — Logo icon changes dynamically based on the active view: sparkles (`✨`) for home/landing page, magnifying glass (`🔍`) for Searching, and bar chart (`📊`) for Sorting.
- **Synchronized Pause Timer** — Real-time execution timer that freezes in sync when the visualization is paused.
...
```
algoviz/
├── index.html                   # HTML mount point (index.html with SEO configuration)
├── package.json                 # Project configuration, scripts, and dependencies
├── vite.config.js               # Vite configurations with React plugin
├── public/                      # Static assets
└── src/
    ├── main.jsx                 # Application entry point
    ├── App.jsx                  # Main application component & view-state router
    ├── index.css                # Ported global styles, colors, and layouts
    │
    ├── components/
    │   ├── Header.jsx           # Application header with navigation, theme, and sound toggles
    │   └── ComplexityTable.jsx  # Dynamic complexity table overlay
    │
    ├── pages/
    │   ├── LandingPage.jsx      # Navigation Hub with card selectors
    │   ├── SortingVisualizer.jsx# View for sorting visualizer controls and bar grid
    │   └── SearchingVisualizer.jsx# View for searching visualizer controls and bar grid
    │
    ├── utils/
    │   └── sound.js             # Web Audio API tone synthesizer (singleton)
    │
    └── algorithms/
        ├── sorting.js           # Bubble, Selection, Insertion, Merge, Quick, Heap implementations
        └── searching.js         # Linear and Binary Search implementations
```
...
3. **Start the local development server**
   ...
   npm run dev
   ```
   Open your browser and visit the local address shown in your terminal (typically `http://localhost:5173`).

4. **Build for production**
   ```bash
   npm run build
   ```
   The compiled static files will be output to the `/dist` directory.
